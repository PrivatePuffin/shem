...
func processFiles(tmpDir string, outputDir string) error {
	...
	for _, file := range files {
		...
		fileName := strings.Replace(file.Name(), "shem_", "", 1)
...
func addYamlTitle(content []byte, isPrimaryIndex bool) []byte {
	...
	if isPrimaryIndex {
		...
		title = strings.TrimPrefix(title, "shem ")
...
// renameshemToIndex renames shem.md to index.md in the given directory.
func renameshemToIndex(dir string) error {
	oldPath := filepath.Join(dir, "shem.md")
	newPath := filepath.Join(dir, "index.md")

	// Check if shem.md exists
	if _, err := os.Stat(oldPath); os.IsNotExist(err) {
		log.Warn().Str("file", oldPath).Msg("shem.md does not exist")
		return nil // No error, just return since file doesn't exist
	}

	// Rename shem.md to index.md
