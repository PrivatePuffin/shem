...
func IsFileIgnored(file string) (bool, error) {
	...
	// Check if the file is ignored with "shem/" prefix
	prefixedFile := "shem/" + file
...
// for both the unprefixed path and the path prefixed with /shem.
...
func IsFileFullyStaged(filePath string) (bool, error) {
	...
	// Check if the shem directory exists in the Git root
	shemPath := filepath.Join(gitRoot, "shem")
	_, err = exec.Command("test", "-d", shemPath).Output()
	shemExists := (err == nil)

	// Create a slice of file paths to check
	filePaths := []string{filePath}
	if shemExists {
		filePaths = append(filePaths, "shem/"+filePath)
	}
